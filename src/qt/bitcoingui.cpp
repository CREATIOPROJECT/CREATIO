...
BitcoinGUI::BitcoinGUI(QWidget *parent):
    ...
    rpcConsole(0),
    blockBrowser(0),
	stakeForMultisendDialog(0)
{
    updateStyle();
    resize(920, 780);
    setWindowTitle(tr("Creatio") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
    qApp->setWindowIcon(QIcon(":icons/micon"));
    setWindowIcon(QIcon(":icons/micon"));
#else
	...
	onlineWindow = new OnlineWindow(this);
    transactionsPage = new QWidget(this);
    QVBoxLayout *vbox = new QVBoxLayout();
    transactionView = new TransactionView(this);
    vbox->addWidget(transactionView);
    transactionsPage->setLayout(vbox);
	creationwindowPage = new Creationwindow(this);
	...
	stakeForMultisendDialog = new StakeForMultisendDialog(this);
	...
	centralWidget->addWidget(stakeForMultisendDialog);
    centralWidget->addWidget(blockBrowser);
	centralWidget->addWidget(onlineWindow);
	centralWidget->addWidget(creationwindowPage);
    ...
    {
        progressBar->setStyleSheet("QProgressBar { background-color: #272727; border: 1px solid grey; border-radius: 7px; padding: 1px; text-align: center; } QProgressBar::chunk { background: QLinearGradient(x1: 0, y1: 0, x2: 1, y2: 0, stop: 0 #FF8000, stop: 1 orange); border-radius: 7px; margin: 0px; }");
    }
	...
	stakeForMultisendDialog = new StakeForMultisendDialog(this);
    connect(multisendAction, SIGNAL(triggered()), stakeForMultisendDialog, SLOT(show()));
...
void BitcoinGUI::createActions()
{
    ...
    overviewAction->setToolTip(tr("General overview"));
    overviewAction->setCheckable(true);
    overviewAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_1));
    tabGroup->addAction(overviewAction);

    sendCoinsAction = new QAction(QIcon(":/icons/send2"), tr("&Send"), this);
    sendCoinsAction->setToolTip(tr("Send coins to a XCRE address"));
    ...
    onlineAction = new QAction(QIcon(":/icons/social"), tr("& Online"), this);
    onlineAction->setToolTip(tr("Links related to Creatio"));
    onlineAction->setCheckable(true);
    tabGroup->addAction(onlineAction);
	
	creationwindowAction = new QAction(QIcon(":/icons/create"), tr("&Create"), this);
    creationwindowAction->setToolTip(tr("Create your own coin"));
    creationwindowAction->setCheckable(true);
    creationwindowAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_7));
	tabGroup->addAction(creationwindowAction);
	...
	connect(onlineAction, SIGNAL(triggered()), this, SLOT(gotoOnlinePage()));
	connect(creationwindowAction, SIGNAL(triggered()), this, SLOT(gotoCreationwindowPage()));
	connect(creationwindowAction, SIGNAL(triggered()), this, SLOT(showNormalIfMinimized()));
    
    quitAction = new QAction(QIcon(":/icons/quit"), tr("E&xit"), this);
    quitAction->setToolTip(tr("Quit application"));
    quitAction->setShortcut(QKeySequence(Qt::CTRL + Qt::Key_Q));
    quitAction->setMenuRole(QAction::QuitRole);
    aboutAction = new QAction(QIcon(":/icons/micon"), tr("&About Creatio"), this);
    aboutAction->setToolTip(tr("Show information about Creatio"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(QIcon(":/trolltech/qmessagebox/images/qtlogo-64.png"), tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(QIcon(":/icons/settings2"), tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for Creatio"));
    optionsAction->setMenuRole(QAction::PreferencesRole);
    toggleHideAction = new QAction(QIcon(":/icons/micon"), tr("&Show / Hide"), this);
    ...
    stakeReportAction->setToolTip(tr("Open the Stake Report"));

    exportAction = new QAction(QIcon(":/icons/export3"), tr("&Export..."), this);
...
void BitcoinGUI::createToolBars()
{
	...
	QToolBar *toolbar2 = addToolBar(tr("Creatio top-logo"));
	...
	toolbar->addAction(creationwindowAction);
	toolbar->addAction(onlineAction);
	toolbar->addAction(onlineAction);

    QToolBar *toolbar3 = addToolBar(tr("Bottom toolbar"));
	addToolBar(Qt::BottomToolBarArea,toolbar3);
	...
	toolbar3->setIconSize(QSize(20,20));
	...
	QToolBar *toolbar4 = addToolBar(tr("Creatio side-logo"));
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("Creatio client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::setWalletModel(WalletModel *walletModel)
{
    ...
    if(walletModel)
    {
		...
		stakeForMultisendDialog->setModel(walletModel);
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("Creatio client"));
	...
	trayIconMenu->addAction(creationwindowAction);
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setPixmap(QIcon(icon).pixmap(40,40));
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to the Creatio network", "", count));
}
...
void BitcoinGUI::setNumBlocks(int count, int nTotalBlocks)
{
    ...
    if(count < nTotalBlocks)
    {
        ...
        if (strStatusBarWarnings.isEmpty())
        {
            ...
            progressBarLabel->setVisible(false);
            progressBar->setFormat(tr("Synchronizing with the Creatio network:   %n block(s) remaining", "", nRemainingBlocks));
    ...
    if(secs < 90*60 && count >= nTotalBlocks)
    {
        ...
        labelBlocksIcon->setPixmap(QIcon(":/icons/synced").pixmap(40,40));
...
void BitcoinGUI::updateStakingIcon()
{
    ...
    {
        labelStakingIcon->setToolTip(tr("Not minting because wallet is locked.<br>Network weight is %1").arg(nNetworkWeight));
        labelStakingIcon->setEnabled(false);
        labelStakingIcon->setPixmap(QIcon(":/icons/staking_off").pixmap(STATUSBAR_ICONSIZE, STATUSBAR_ICONSIZE));
    }
    else if (vNodes.empty())
    {
        labelStakingIcon->setToolTip(tr("Not minting because wallet is offline.<br>Network weight is %1").arg(nNetworkWeight));
        labelStakingIcon->setEnabled(false);
        labelStakingIcon->setPixmap(QIcon(":/icons/staking_off").pixmap(STATUSBAR_ICONSIZE, STATUSBAR_ICONSIZE));
    }
    else if (IsInitialBlockDownload())
    {
        labelStakingIcon->setToolTip(tr("Not minting because wallet is syncing.<br>Network weight is %1").arg(nNetworkWeight));
        labelStakingIcon->setEnabled(false);
        labelStakingIcon->setPixmap(QIcon(":/icons/staking_off").pixmap(STATUSBAR_ICONSIZE, STATUSBAR_ICONSIZE));
    }
    else if (!nWeight)
    {
        labelStakingIcon->setToolTip(tr("Not minting because you don't have mature coins.<br>Network weight is %1").arg(nNetworkWeight));
        labelStakingIcon->setEnabled(false);
        labelStakingIcon->setPixmap(QIcon(":/icons/staking_off").pixmap(STATUSBAR_ICONSIZE, STATUSBAR_ICONSIZE));
		labelStakingIcon->setStyleSheet("QStatusBar{ background-color: #272727;}");
    }
    ...
    {	
        ...
        labelStakingIcon->setToolTip(tr("Minting.<br>Your weight is %1.<br>Network weight is %2.<br><b>Estimated</b> next stake in %3").arg(nWeight).arg(nNetworkWeight).arg(text));
    }
    else
    {
...
void BitcoinGUI::gotoOnlinePage()
{
    onlineAction->setChecked(true);
    centralWidget->setCurrentWidget(onlineWindow);
 ...
 void BitcoinGUI::gotoCreationwindowPage()
 {
     creationwindowAction->setChecked(true);
     centralWidget->setCurrentWidget(creationwindowPage);
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid Creatio address or malformed URI parameters."));
    }
...
void BitcoinGUI::setEncryptionStatus(int status)
{
    ...
    case WalletModel::Unlocked:
        ...
        labelEncryptionIcon->setPixmap(QIcon(":/icons/lock_open").pixmap(40,40));
    ...
    case WalletModel::Locked:
        labelEncryptionIcon->show();
        labelEncryptionIcon->setPixmap(QIcon(":/icons/lock_closed").pixmap(40,40));
...
void BitcoinGUI::updateStyle()
{
    ...
    QString qssPath = QString::fromStdString( GetDataDir().string() ) + "/CreatioQSSv1.qss";
    ...
    {
        qDebug() << "failed to open stayle sheet";
        return;
    }
