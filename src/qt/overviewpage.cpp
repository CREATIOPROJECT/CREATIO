    ...
    filter(0)
{
    ...
    ui->labelWalletStatus->setText("(" + tr("Out of sync with the Creatio network") + ")");
    ui->labelTransactionsStatus->setText("(" + tr("Out of sync with the Creatio network") + ")");
    ...
    if(GetBoolArg("-chart", true))
    {
        ...
        ui->diffplot->yAxis->setLabel("24H Net Weight");
...
void OverviewPage::updatePlot(int count)
{
    ...
    if(fDebug) { printf("Plot: Getting Ready: pindexBest: %p\n", pindexBest); }
		...
		bool fProofOfStake = (nBestHeight > 50);
    if (fProofOfStake)
        ui->diffplot->yAxis->setLabel("24H Net Weight");
		else
...
void OverviewPage::setModel(WalletModel *model)
{
    ...
    // update the display unit, to not use the default ("XCRE")
    updateDisplayUnit();
}
